...
function addTodo() {
  ...
  const sidenotes = document.createElement("input");
  ...
  if (textBox.value !== "") {
    ...
    sidenotes.setAttribute("type", "text");
    sidenotes.setAttribute("placeholder", "Notes");

    newTodoContainer.appendChild(sidenotes);
...
function actionButton(button, action) {
  ...
  const sidenotes = todoContainer.querySelector("input");
  ...
  // Save sidenotes to local storage
  if (sidenotes) {
    saveTodos();
  }
}
...
function saveTodos() {
  ...
  for (const container of containers) {
    ...
    const sidenotes = container.querySelector("input");
    if (paragraph) {
      const todoText = paragraph.textContent;
      const isChecked = paragraph.classList.contains("checked");
      const noteText = sidenotes ? sidenotes.value : "";
      todos.push({ text: todoText, checked: isChecked, note: noteText });
    }
  }
...
function loadTodos() {
  ...
  if (savedTodos) {
    ...
    for (const todo of todos) {
      ...
      sidenotes.setAttribute("type", "text");
      sidenotes.setAttribute("placeholder", "הערות");
      sidenotes.value = todo.note;
